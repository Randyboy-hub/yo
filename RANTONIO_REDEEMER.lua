...
local function isOurGui(instance)
    ...
    for _ = 1, 10 do
        ...
        if p.Name == "RANTONIORedeemerUI" or p.Name == "SourcesHubRedeemerGui" then return true end
        p = p.Parent
    end
    return false
end
...
local COLORS = {
    ...
    White = Color3.fromRGB(0, 170, 255),
...
-- CLEANUP OLD GUIS --
pcall(function()
    for _, name in ipairs({"RANTONIORedeemerUI", "AutoTypeCodesUI", "ACEPaste"}) do
...
makeLabel(Header, "Title", "RANTONIO REDEEMER", UDim2.fromOffset(180, 25), UDim2.fromOffset(56, 17), 15, COLORS.White, Enum.Font.GothamBold)
...
if aceNotifyRemote then
    if getgenv then
        local previous = getgenv().RANTONIORedeemerNotifyConnection
    ...
    if getgenv then getgenv().RANTONIORedeemerNotifyConnection = aceListenConnection end
end
...
if getgenv then
    getgenv().StopAura = function()
        ...
        if getgenv().RANTONIORedeemerNotifyConnection then
            pcall(function() getgenv().RANTONIORedeemerNotifyConnection:Disconnect() end)
            getgenv().RANTONIORedeemerNotifyConnection = nil
        end
